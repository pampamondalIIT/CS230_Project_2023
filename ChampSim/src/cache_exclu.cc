...
//Function  called to fill the cache  when response received from the lower level cache  for a required memory block.

void CACHE::handle_fill()
{
    // handle fill   
    //  from which CPU the request came. cache of which core. 
...
//checking if the request is valid
        ...
        // find victim
        // MSHR.entry[mshr_index].address - full address of the block  we missed in the cache  
        //and wants to fetch it from lower  cache or memory
...
//------------------------------------------------
// LLC is not filled in exclusive cache hierarchies only L1  and L2 are filled 
        //When a cache block is evicted from the last level cache, the cache controller first checks whether the block is present in any other caches in the system. 
        //If the block is found in another cache, the cache controller sends an invalidation message to the other cache instructing it to evict the block. The block is then removed from the other cache, and the new block can be inserted into the last level cache.
        if (cache_type == IS_LLC) {
        ...
        //If the victim block is dirty (lower level cache should know it's value before it is evicted), the function checks if there is enough space in the lower level cache's write queue to accommodate the dirty block before replacing it with the incoming block.
        // If there is not enough space, the function sets a flag do_fill to 0 and increments the STALL counter for the corresponding miss type. 
        //If there is enough space, the function constructs a writeback packet and adds it to the lower level cache's write queue.

        else if (cache_type == IS_L2C) {
            // do not writeback dirty blocks now, they will be handled later
            if (block[set][way].valid && !block[set][way].dirty) {    //block is being written newly
...
//It handles the writeback of a cache block, which occurs when a dirty block is being evicted from the cache to a lower level of the memory hierarchy.
//If the request is valid, the function proceeds to handle the writeback. It first accesses the cache to check if the block to be written back is present in the cache. 
//If it is, then the block is marked as dirty, the cache replacement state is updated, and statistics related to cache hits are updated.

//If the block to be written back is not present in the cache, then it is a miss. In the case of a cache miss, the function checks the MSHR (miss status handling register) to see if a previous miss request for the same block is already in progress. 
//If it is, then the function updates the fill level of the previous miss request. If not, then the function adds the miss request to the MSHR and adds it to the read queue of the lower level cache, if one exists
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                // L2 evictions are sent into LLC  to write 
                if (cache_type == IS_L2C) {
                    // do not writeback dirty blocks now, they will be handled later
                    if (block[set][way].valid && !block[set][way].dirty) {
                        if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
                           ...
                           uint8_t   do_fill = 0;
                            lower_level->increment_WQ_FULL(block[set][way].address);
                            STALL[WQ.entry[index].type]++;     //make stall
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // When it is a read hit
		  ...
		  else
		    {
		      ...
		      if (RQ.entry[index].is_data)
                ...
                RQ.remove_queue(&RQ.entry[index]);
		            reads_available_this_cycle--;    //decreasing reads_available due to hit 
//-------------------------------------------------------------------
...
//returning the data to the processor.
