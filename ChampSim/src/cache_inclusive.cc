...
//This function is used to check if any higher level cache is holding a dirty copy of the block corresponding to a given address. It is called recursively for each level of the cache hierarchy, starting from the LLC (last-level cache) and going up to the L1 cache. 
//because we know if a address Y is to be evicted from LLC and it is present in L1 or L2 then all should be invalidated. 
...
//making all the entries invalidate 

void CACHE::make_all_invalidate(uint64_t address)
{
    uint8_t upper_level_dirty = 0;
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        //  the block we are filling is this dirty?
    ...
    //------------------------------------------------------------------------------
        ...
        //------------------------------------------------------------------------
                    make_all_invalidate(block[set][way].address);
        //--------------------------------------------------------
